...
def generate_with_start(model, length, vocab_size, ix_to_char, start):
    ix = start
    ...
    for i in range(length):
        ...
        ix = np.argmax(model.predict(X[:, :i+1, :], batch_size=1)[0], 1)
        y_char.append(ix_to_char[ix[-1]])
    return ('').join(y_char)

def generate_with_start_random(model, length, vocab_size, ix_to_char, start):
    ix = start
    y_char = [ix_to_char[ix[-1]]]
    X = np.zeros((1, length, vocab_size))
    for i in range(length):
        ...
        chosen = np.where(normalized>=random)[-1][0]
        y_char.append(ix_to_char[chosen])
    return ('').join(y_char)
